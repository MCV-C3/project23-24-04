...
PATCH_SIZE  = 32
PATCHES_DIR = '/home/gherodes/projects/tf_test/MIT_split_patches'+str(PATCH_SIZE)
...
filename = f'patches_{model_type}-{num_units}-{img_size}-{reg_coeff}'
...
if model_type == 'no_residuals':
    model = build_mlp(img_size,
                    ...
                    filename=f'{experiment_directory}/model_{filename}.png'
                )
...
LEARNING_RATE, decay_steps=0.75 * EPOCHS*BATCH_SIZE//1881, alpha=0.1)

learning_rates = [lr_schedule(step).numpy() for step in range(EPOCHS*BATCH_SIZE//1881)]
# Plot the learning rates
plt.plot(range(EPOCHS*BATCH_SIZE//1881), learning_rates, label='Learning Rate')
